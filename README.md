...
class MySQLMATEDATA():
    ...
    def camel_to_snake(name):
        ...
        :return: 新的列名称
        """
        snake_case = re.sub(r'(?, '_', name).lower()
        return snake_case
...
| complete\_regex | 是否完整匹配表名称，如果为true，则根据key完整匹配表名称，用于解决分库分表的问题， 如果为false，则根据regex的值进行正常匹配 |
...
| config.sink.properties.strip\_outer\_array | 是否展开JSON数组 |
...
1. [python\-mysql\-replication](https://github.com) python实现的用于binlog同步的库。
2. [starrocks\-connector\-for\-kafka](https://github.com) Kafka Connector是StarRocks数据源连接器
3. [DataX](https://github.com) 批量数据同步工具
4. [kafka\-console\-ui](https://github.com):[楚门加速器](https://chuanggeye.com) Kakfa可视化控制台
5. [StarRocks\-kafka\-Connector](https://github.com)  通过kafkaConnector导入数据到StarRocks
6. [StreamLoad实现数据增删改](https://github.com)
...
解决：不要使用最新版本，使用0\.45\.1版本，可参考：[issue\#612](https://github.com)
